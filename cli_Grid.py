...
@click.option(
    '--file', 
    default=None, 
    help='Endereço do seu diretório onde o arquivo contendo as informações do local para gerar o grid está.')
...
def execute(locality, file, resolution, border, filename, verbose, filename_base, filename_int, filename_grid):
    ...
    if filename_int is not None:
        ...
        gpd.GeoSeries(dados_grid['center_point']).plot(ax=ax, color='black', markersize=15, label='Center_Point')
        gpd.GeoSeries(dados_grid['envelope']).plot(ax=ax, color='blue', label='Envelope', alpha=0.25)
    ...
    if filename_grid is not None:
        ...
        gpd.GeoSeries(grid_filtrado['center_point']).plot(ax=ax, color='black', markersize=15, label='Center_Point')
        gpd.GeoSeries(grid_filtrado['envelope']).plot(ax=ax, color='blue', label='Envelope', alpha=0.25)
...
--filename_int figura1 
--filename_grid figura1 
--resolution 0.5 
--border 0.5
"""
